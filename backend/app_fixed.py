#!/usr/bin/env python3
"""
MediMotion Backend - Fixed for MediaPipe 0.10.35+
"""
...
print("=" * 60)
print("MediMotion Backend Starting")
print("=" * 60)
...
# MediaPipe 0.10.35+ import syntax
try:
    import mediapipe as mp
    print(f"✓ MediaPipe version: {mp.__version__}")
    
    # New import syntax for MediaPipe
...
# Initialize MediaPipe Pose (using legacy API which still works)
if MP_AVAILABLE:
    try:
        ...
        if hasattr(mp, 'solutions'):
            ...
            print("✓ MediaPipe Pose initialized (legacy API)")
        else:
            # Use the new pose landmarker
            print("⚠ Legacy MediaPipe API not available, using basic mode")
            pose = None
            mp_drawing = None
    except Exception as e:
        print(f"⚠ Could not initialize MediaPipe Pose: {e}")
        pose = None
        mp_drawing = None
...
def health_check():
    ...
    return jsonify({
        ...
        'message': 'MediMotion Backend is running!'
    })
...
def analyze_pose():
    ...
    try:
        ...
        if MP_AVAILABLE and pose is not None:
            try:
                # Convert to RGB
                ...
                if results.pose_landmarks:
                    ...
                    # Draw landmarks if drawing utilities are available
                ...
                feedback_text.append("Processing error. Please try again.")
                overall_feedback = "System is ready. Please ensure good lighting."
                cv2.putText(frame, "Processing...", (10, 30),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
        else:
            # Fallback when MediaPipe is not available
            feedback_text.append("Pose detection system is ready!")
            overall_feedback = "Start your exercise! Keep your full body in frame."
            ...
            if exercise_type == 'shoulder_raise':
                angles = {'left_shoulder': 145, 'right_shoulder': 148}
            elif exercise_type == 'elbow_curl':
                angles = {'left_elbow': 155, 'right_elbow': 152}
            elif exercise_type == 'knee_bend':
                angles = {'left_knee': 135, 'right_knee': 138}
